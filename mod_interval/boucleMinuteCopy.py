...
def scanDevices():
	...
	for dev in devices:
		...
		for (adtype, value) in dev.getScanData():
			#pour chaque ligne dans device, ajout clé adtype, valuer : value
			dicoDevices.update({adtype: value})


		#nom du device à récuperer dans le atype8
		name = dicoDevices.get(8)

		#traitement des données du capteur à récuperer dans le adtype 22
		valueAdtype22 = dicoDevices.get(22)
			#if adtype == 8:
			#	name = value
		#
			#if adtype == 22:
		if len(valueAdtype22) == 38 and valueAdtype22[4:10] == "113901" and valueAdtype22[32:39] == "000000" :
			if 11000000 < int(valueAdtype22[12:20]) < 12000000:
				# save l'index correspondant au numéro d'identification dans var valeurId
				valeurId = valueAdtype22[12:20]
				# save l'index correspondant à la valeur batterie dans var valeurBatterieHexa
				valeurBatterieHexa = valueAdtype22[20:22]
				# save l'index correspondant à la valeur température dans var valeurtempHexa
				valeurTempHexa = valueAdtype22[24:28]
				...
				if valeurTemp < 2400 or valeurTemp > 3000 :
					...
					if name not in dicoTemperature :
						# si le capteur n'est pas dedans, il ajoute le capteur et sa temp dedans puis envoi le mail messageKO de danger
						dicoTemperature.update({name: valeurTemp})
						envoiMail(name, valeurId, temperature, valeurBatterie, bool(True))
				...
				# si c'est compris dans le seuil
					...
					if name in dicoTemperature :
						# si capteur est déjà existant, supprime ses données du dico et envoie un mail messageOK de rétablissement
						del dicoTemperature[name]
						envoiMail(name, valeurId, temperature, valeurBatterie, bool(False))
			...
			print("probleme if len(valueAdtype22)")
